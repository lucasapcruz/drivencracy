...
export async function defineChoice(req, res) {
    ...
    try {
        ...
        const choiceTitleOccInPoll = await choices.find({
            $and:
                [{ title }, { pollId: new ObjectId(pollId) }]
        }).toArray();
        
        
        if (choiceTitleOccInPoll.length > 0) {
            res.sendStatus(409);
            return;
        }
        ...
        await choices.insertOne(choice);
...
export async function voteOnChoice(req, res) {
    ...
    try {
        ...
        const poll = await polls.findOne({
            _id: new ObjectId(choiceExists.pollId),
        });
